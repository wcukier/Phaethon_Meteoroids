...
if (__name__ == "__main__"):
        ...
        for l in range(10):
                ...
                np.save(f"{dr}/output/{subdir}/beta{10*k+l}.npy", beta)
                np.save(f"{dr}/output/{subdir}/mass{k*10+l}.npy", mass)

                for i in tqdm(range(int((n_years-window)))):
                        sim.integrate(int(i*year))
                    
                sim.save(f"{dr}/output/{subdir}/sim{k*10+l}.bin")
                ...
                np.save(f"{dr}/output/{subdir}/particles{k*10+l}.npy", xy)
